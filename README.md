...
[![Build](https://img.shields.io/badge/build-v0.3.1-blue)](https://github.com/HuckleR2003/ScalingLaws-AI/releases)
[![Tests](https://img.shields.io/badge/tests-1418%20EditMode%20%2B%2065%20PlayMode-brightgreen)](#testing)
...
in 2023 and never revisited is quietly throttling by 2027 without you having touched anything. And the
room itself has a heat budget: every cabinet sheds into the same air, so past it all of them cool
worse at once, and the fix takes two squares of floor you wanted for another rack.
...
you fill yourself, card by card. Renting tracks whatever the clouds offer, 180 days behind launch, and
never ages. Owning is roughly a third of the price per FLOP and bills whether the cluster is busy or
idle. A bigger building buys cheaper power: $0.19 a kWh at home, $0.14 in colocation, $0.055 in your
own datacenter. Every cabinet says how many people it keeps served.
...
**Rivals** are fourteen labs, thirteen seeded from the real 2022 to 2026 release timeline and then
free to deviate, and one invented: the player's cousin's small company in Radom. A patient lab that sees better silicon landing shortly after its planned launch will hold,
...
**Research** is a tree of 64 nodes across five eras and five tracks (capability, model improvement,
safety, operations and premises). Every architecture, corpus,
...
mail. Plus a model creator in eight stages, a server room you build and fill by hand, rented offices
with your company's name over the door, and a city map of Bayview with the places a lab grows into.
...
A fresh clone shows the office furniture, the server racks and the character models as missing
references. Those are Asset Store packs and CGTrader models; their licences forbid redistribution, so
they are gitignored. Every room falls back to plain shapes without them, and everything that decides
anything is in the repository. The photographed textures (ambientCG) and furniture (Poly Haven) are
CC0 and are included.
...
| `Assets/_ScalingLaws/Tests/EditMode/` | 1418 tests across 163 fixtures. None load a scene. |
| `Assets/_ScalingLaws/Tests/PlayMode/` | 65 tests that do load a scene, and render pages to PNG |
...
Saves carry a version and **one migration step per version**, currently at v62. Old shapes are kept
...
**Source available, all rights reserved.** See [LICENSE.md](LICENSE.md). You are welcome to read the
code, learn from it, build it on your own machine and quote it with a link. You may not distribute it,
or a build or game made from it, without written permission: kematex2202@gmail.com.

The game itself is free to play from the builds published on the
[Releases](https://github.com/HuckleR2003/ScalingLaws-AI/releases) page and on itch.io.

Versions published before 19 September 2026 were released under PolyForm Noncommercial 1.0.0, and
those copies keep that licence.
