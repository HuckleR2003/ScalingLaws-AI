...
namespace ScalingLaws.Simulation
{
    ...
    public sealed class SupportDesk
    {
        ...
        /// </summary>
        public double CapacityHoursPerDay(int supportStaff)
        {
            var people = Math.Max(0, supportStaff) * SupportCatalog.HoursPerPersonPerDay;
        ...
        /// </summary>
        public void Advance(double servedUsers, int supportStaff)
        {
            ...
            var peopleHours = Math.Max(0, supportStaff) * SupportCatalog.HoursPerPersonPerDay;
        ...
        /// </summary>
        public double AverageHours(int supportStaff)
        {
            var capacity = CapacityHoursPerDay(supportStaff);
            if (BacklogHours <= 0.0)
            {
                return 0.0;
            }
        ...
        /// <summary>
        /// Nothing to answer is not a failing desk. One when the post is answered inside
        /// <see cref="SupportCatalog.AnsweredHours"/>, zero past
        /// <see cref="SupportCatalog.AbandonedHours"/>, straight line between.
        /// </summary>
        public double Quality(int supportStaff)
        {
            if (BacklogHours <= 0.0)
            {
                return 1.0;
            ...
            var hours = AverageHours(supportStaff);
            if (hours <= SupportCatalog.AnsweredHours)
            {
                return 1.0;
            }

            var span = SupportCatalog.AbandonedHours - SupportCatalog.AnsweredHours;
            return Math.Clamp((SupportCatalog.AbandonedHours - hours) / span, 0.0, 1.0);
        }
        ...
        /// </summary>
        public double ServiceMultiplier(int supportStaff)
        {
            var quality = Quality(supportStaff);
        ...
        public void Restore(double low, double medium, double high,
